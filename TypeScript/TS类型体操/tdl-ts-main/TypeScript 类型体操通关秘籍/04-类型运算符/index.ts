 ...
 * @LastEditTime: 2022-05-04 13:27:02
 * @LastEditors: raotaohub
 * @FilePath: \TS\TS类型体操\tdl-ts-main\TypeScript 类型体操通关秘籍\04-类型运算符\index.ts
 * @Description:
 */
...
type one = string;
type two = number;
type Cross = one & two; //  never 交叉类型将不同类型舍弃

type one1 = string;
type two1 = string;
type Cross1 = one & two; //  string 同类型合并
...
type MapTypeKeyx3<T> = {
  [key in keyof T as `${key & string /*通过交叉类型同类合并，异类舍弃，只留下string类型的key*/}${key &
    string}${key & string}`]: [T[key], key];
};
